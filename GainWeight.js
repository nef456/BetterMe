...
/*This screen calculates the calories a user needs to gain weight.*/

const GainWeight = ({ navigation, calories, setCalories, basilRate }) => {
  ...
  const calculateCalories = () => {
    ...
    if(activityLevel == 'Extremely Active'){
      ...
      increase = (basilRate * 1.725) + 500;
      setCalories(increase);

    }else if (activityLevel == 'Moderately Active'){

      increase = (basilRate * 1.55) + 500;
      setCalories(increase);

    }else if(activityLevel == 'Lightly Active'){

      increase = (basilRate * 1.375) + 500;
      setCalories(increase);

    }else if(activityLevel == 'Sedentary'){

      increase = (basilRate * 1.2) + 500;
      setCalories(increase);
    ...
    if (activityLevel == ''){
      ...
      setCalories(increase);
      navigation.setParams({calories: increase});
      setError('');
      setActivityLevel('');
    }
    ...
    <Text style={styles.screenHeading}>Gain Weight</Text>
      <Text style={styles.heading}>  {basilRate ? 'Calculated BMR: ' + basilRate.toFixed(1) : ''}</Text>
      <Text style={styles.heading}>If gaining weight is your goal, please enter your activity level below for defined calorie intake results.</Text>
      ...
      <Pressable
        ...
        <Text style={styles.caloriesMessage}>{basilRate ? 'Amount of calories for weight gain: ' + calories.toFixed(1) : ''}</Text>
